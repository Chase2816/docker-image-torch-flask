...
def convert_voc_annotation(anno_path, use_difficult_bbox=True):

    classes = ['diode', 'shadow', 'stain']
    # classes = ['diode', 'shadow']
    # classes = ['fish']

    # img_inds_file = r"E:\pycharm_project\image_aug_for_detection\myfile\trainval.txt"
    # with open(img_inds_file, 'r') as f:
    #     txt = f.readlines()
    #     image_inds = [line.strip() for line in txt]

    xml_path = r"F:\data\M300_opt1228\outputs"
    img_path = r"F:\data\M300_opt1228\images"
    image_inds = [line.split('.')[0] for line in os.listdir(xml_path)]
    print(image_inds)

    with open(anno_path, 'a') as f:
        for image_ind in image_inds:
            image_path = os.path.join(img_path,image_ind+'.jpg')
            # image_path = data_path + 'JPEGImages' +'/' + image_ind + '.jpg'
            annotation = image_path
            label_path = os.path.join(xml_path, image_ind + '.xml')
...
if __name__ == '__main__':
    ...
    num1 = convert_voc_annotation(flags.train_annotation, False)
